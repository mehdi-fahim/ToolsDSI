...
FROM (
    SELECT TRIM(TYPE_PAIEMENT || ' ' || NUM_CHQ) AS TYPE_PAIEMENT,
           MONTANT_REGLE,
           TO_CHAR(DATE_PREENC, 'DD/MM/YYYY') AS DATE_PREENC,
           SAISI_PAR
    FROM PREENC_SAISIE
    WHERE intitule = :intitule
    ORDER BY DATE_PREENC DESC
)
WHERE ROWNUM < 6
SQL,
                ['intitule' => $intitule]
...
SQL,
            ...
            $lastMonth = $this->fetchScalar(
                <<<SQL
SELECT MOIS FROM (
    SELECT TO_CHAR(DATE_PREENC, 'MM') AS MOIS
    FROM PREENC_SAISIE
    WHERE intitule = :intitule
    ORDER BY DATE_PREENC DESC
)
