...
SELECT
  ...
  DECODE(SUBSTR(g.paesi_codext,1,2),
         'IS',93039,'VT',93079,'SD',93066,'PI',93059,'LC',93027,'EP',93031,'AU',93001) L_DEPCOM,
  ...
  NVL(GET_LOYER_FAC(a.glcon_num,a.glcon_numver,LAST_DAY(ADD_MONTHS(SYSDATE, 1))),0) L_LOYER,
  NVL(GET_RLS(a.glcon_num,a.glcon_numver), NVL(GET_SLS(a.glcon_num,a.glcon_numver),0)) L_SURLOYER,
  NVL(GET_CHARGES_FAC(a.glcon_num,a.glcon_numver,LAST_DAY(ADD_MONTHS(SYSDATE, 1))),0) L_CHARGES,
...
FROM opulise.glelc a, opulise.paesi b, opulise.glrfc c, opulise.glrec d, opulise.mgadr e,
     opulise.paifg f, opulise.paesi g, opulise.glcon h, opulise.parel i
WHERE a.paesi_num(+) = b.paesi_num
  ...
  AND b.paesi_codext IN (SELECT ESI FROM ETUDES.INSEE_ESI WHERE ANNEE = 2022)
UNION
SELECT
  GET_RPLS(b.paesi_num) L_IDENT_REP,
  b.paesi_codext L_IDENT_INT,
...
UNION
SELECT
    '0007527833'  AS L_IDENT_REP,
    'SD0092.02.01.0007' AS L_IDENT_INT,
    93066 AS L_DEPCOM,
    '4 RUE GEORGES POLITZER 93200 ST DENIS' AS L_ADRESSE,
    '03' AS L_ETAGE,
    'C' AS L_TYPECONST,
    '3' AS L_NBPIECE,
    62.00 AS L_SURFACEHAB,
    111.00 AS L_SURFACECOR,
    '1976' AS L_CONSTRUCT,
    1 AS L_WC,
    1 AS L_SANIT,
    1 AS L_CHAUF,
    ...
    246.07 AS L_CHARGES,
    0.00 AS L_AIDES,
    '202601' AS L_DATELOYER,
    '1' AS L_PERIODICITE,
    '20240401' AS L_DATEENTREE,
    2 AS L_VACANT,
    '99999999' AS L_DATESORTIE
FROM DUAL
ORDER BY 2
SQL;
